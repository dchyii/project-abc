...
with col2:
    ...
    if prompt := st.chat_input("What is up?"):
        ...
        ##! confirmation of the main headers of the scope of work!##
        ...
        ##! confirmation of draft
        elif st.session_state.stage["current"] == st.session_state.stage["max"] and st.session_state.stage["status"] == "confirmed":
            # Display user message in chat message container
            chatcontainer.chat_message("user").markdown(prompt)
            # Add user message to chat history
            st.session_state.messages.append({"role": "user", "content": f"{prompt}" })
            ...
            section_list = json.loads(st.session_state.background[1]["content"])
            current_section = section_list[st.session_state.stage["current"] - 1]
            system_instructions = [{"role": "system", "content": f"You are currently drafting the requirements specifications for the {current_section} section. " + system_prompts.system_prompts["for_confirmation"]}]
